...
void BotControl::pidAlgorithm(){
	...
	double Kp_f_High = 100;
	double Kp_f_Low  = 1;
	double Ki_f_High = 0.2;
	double Ki_f_Low  = 0;
	double Kd_f_High = 0;
	double Kd_f_Low  = 0;
	...
	double Kd_a_High = 0;
	double Kd_a_Low  = 0;
	double SpeedDown_distance = 2;
	double SpeedDown_angle = 0.1;
	
	if (error_angle_ < -SpeedDown_angle || error_angle_ > SpeedDown_angle)
	{
		trans_forward_ = error_forward_ * Kp_f_Low + I_forward_ * Ki_f_Low + D_forward_ * Kd_f_Low;		// Low gain  (motoring)
		trans_angle_ = error_angle_ * Kp_a_High + I_angle_ * Ki_a_High + D_angle_ * Kd_a_High; 			// High gain (steering)
	} else if (error_forward_ > SpeedDown_distance){
		trans_forward_ = error_forward_ * Kp_f_High + I_forward_ * Ki_f_High + D_forward_ * Kd_f_High;	// High gain (motoring)
		trans_angle_ = error_angle_ * Kp_a_Low + I_angle_ * Ki_a_Low + D_angle_ * Kd_a_Low;				// Low gain  (steering)
	} else {
		trans_forward_ = error_forward_ * Kp_f_Low + I_forward_ * Ki_f_Low + D_forward_ * Kd_f_Low;		// Low gain  (motoring)
		trans_angle_ = error_angle_ * Kp_a_Low + I_angle_ * Ki_a_Low + D_angle_ * Kd_a_Low;				// Low gain  (steering)
	}
